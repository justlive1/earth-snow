...
public class ThreadUtils {
   ...
   * @return 任务池
   */
  public static ScheduledExecutorService newSecheduledExecutor(int corePoolSize,
      String nameFormat) {
    return newSecheduledExecutor(corePoolSize, nameFormat, new ThreadPoolExecutor.AbortPolicy());
  }
   ...
   * @return 任务池
   */
  public static ScheduledExecutorService newSecheduledExecutor(int corePoolSize, String nameFormat,
      RejectedExecutionHandler handler) {
  ...
  private static class RetryTask implements Callable<Boolean> {
    ...
    public Boolean call() throws Exception {
      log.info("重试任务开始，{}, limit:[{}]", callable, limit);
      try {
        callable.call();
        return true;
      } catch (Exception e) {
        if (limit > 0) {
          log.error("重试任务-任务失败-重试剩余次数-{}", limit, e);
          INNER_EXECUTOR.submit(new RetryTask(--limit, callable, failCall));
        } else {
          log.error("重试任务-任务失败-重试超限", e);
          if (failCall != null) {
            failCall.call();
          }
        }
